...
public class doctorcontroller {
    ...
    public ResponseEntity<Doctor> updateDoctorColumn(@PathVariable String id, @PathVariable String column, @PathVariable String data) {
        return doc_serv.updateDoctorColumn(id,column,data);
    }

    @PostMapping("/doctors/{id}/photo")
    public ResponseEntity<String> addPhotoToDoctor(@PathVariable String id, @RequestParam("file") MultipartFile file) throws IOException {
        return doc_serv.addPhotoToDoctor(id,file);
    }

    @GetMapping("/doctors/{id}/photo")
    public ResponseEntity<byte[]> getDoctorPhoto(@PathVariable String id) {
